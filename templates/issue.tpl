...
<a href="javascript:addAllArticles()" id="podAddAll">{translate key="plugins.generic.pod.addAll"}</a>
	...
	<td class="tocGalleys">
		{if $hasAccess || ($subscriptionRequired && $showGalleyLinks)}
			{foreach from=$article->getLocalizedGalleys() item=galley name=galleyList}
				<!-- Here comes the epublic part -->
				{if $galley->isPdfGalley()}
					<a href="{url page="article" op="view" path=$articlePath|to_array:$galley->getBestGalleyId($currentJournal)}" class="filePod" title="{translate key="plugins.generic.pod.showArticlePDFMouseover"}">
						<img src="{$baseUrl}/plugins/generic/pod/icons/pdf-icon.png"/></a>
					<a id="{$articlePath}/{$galley->getBestGalleyId($currentJournal)}" title="{translate key="plugins.generic.pod.addArticleMouseover"}" class="podAdd" href="javascript:addPodArticle('{$articlePath}/{$galley->getBestGalleyId($currentJournal)}')" disabled="">
						<img id="img-add-{$articlePath}/{$galley->getBestGalleyId($currentJournal)}" src="{$baseUrl}/plugins/generic/pod/icons/doc-option-add.png"/></a>
					<a id="rem-{$articlePath}/{$galley->getBestGalleyId($currentJournal)}" href="javascript:removePodArticle('{$articlePath}/{$galley->getBestGalleyId($currentJournal)}')" title="{translate key="plugins.generic.pod.removeArticleMouseover"}" class="podRemove" disabled="">
						<img id="img-rem-{$articlePath}/{$galley->getBestGalleyId($currentJournal)}" src="{$baseUrl}/plugins/generic/pod/icons/doc-option-remove.png"/></a>
				{/if}
				<!-- end of epublic part -->
